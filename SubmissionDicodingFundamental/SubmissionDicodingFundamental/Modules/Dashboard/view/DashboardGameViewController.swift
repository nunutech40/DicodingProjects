...
class DashboardGameViewController: UIViewController {
    ...
    @objc func didSearch(_ sender: UITapGestureRecognizer) {
        ...
        self.viewModel.page = 1
        self.viewModel.dataModel = []
        guard self.searchTf.text != "" && self.searchTf.text != nil else {
            let alert = UIAlertController(title: "Error Search", message: "Search Tidak Boleh Kosong!", preferredStyle: UIAlertController.Style.alert)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if position > (tableView.contentSize.height - 100 - scrollView.frame.size.height) {
            if self.viewModel.isHasLoad == false {
                ...
                print("cek halaman ke: \(self.viewModel.page)")
